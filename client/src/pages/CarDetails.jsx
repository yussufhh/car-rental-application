      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12'>
              ...
              <ul className='grid grid-cols-1 sm:grid-cols-2 gap-2'>
                {["360 Camera", "Bluetooth", "GPS", "Heated Seats", "Rear View Mirror"].map((feature) => (
                  <li key={feature} className='flex items-center text-gray-500'>
                    <img src={assets.check_icon} className='h-4 mr-2' alt="check_icon" />
                    {feature}
                  </li>
                ))}
              </ul>
        ...
        <form onSubmit={handleSubmit} className='shadow-lg h-max sticky top-18 rounded-x1 p-6 space-y-6 text-gray-500'>
          ...
          <div className='flex flex-col gap-2'>
            ...
            <input type="date" id="pickup-date" className='border border-borderColor px-3 py-2 rounded-xl'
              required min={new Date().toISOString().split('T')[0]} />

          </div>
            <div className='flex flex-col gap-2'>
            <label htmlFor="return-date" className='block mb-2'>Return Date</label>
            <input type="date" id="return-date" className='border border-borderColor px-3 py-2 rounded-xl'
              required />
